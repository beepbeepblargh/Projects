<# Sharepoint_Policy_Creator.ps1
Written by Chris Ng
.DESCRIPTION
This script is intended to create conditional access policies for blocking Sharepoint on unmanaged devices.
.Version history
v.1.0 - Single and MultiMode parameters for doing Bulk or Single URL policy creation. 
#>
...
#$sharepointurl = "https://n0151c-admin.sharepoint.com"
$sharepointurl = "https://runtimecollective-admin.sharepoint.com"
# Check if more than one mode switch is used
if ($Multimode -and $Singlemode) {
    Write-Host "Error: Both -Multimode and -Singlemode cannot be used together."
    exit
}


# Check if neither mode switch is used
if (-not ($Multimode -or $Singlemode)) {
    Write-Host "Error: Please specify either -Multimode or -Singlemode."
    exit
}

function Connect_SharepointOnline {
    $module = Get-Module -Name Microsoft.Online.Sharepoint.Powershell -ListAvailable
    if ($module.count -eq 0) {
        Write-Host "Installing Sharepoint Online..."
        Install-Module -Name Microsoft.Online.Sharepoint.Powershell
        if ($module.count -eq 1) {
            Write-Host "Sharepoint Online installed"
        }
...
Write-Host "Preparing Sharepoint Online Connection"
Connect_SharepointOnline


if ($Multimode) {
    Write-Host "Performing bulk-Sharepoint policy creation"
...
elseif ($Singlemode) {
    Write-Host "Attempting to retrieve list of personal sharepoints. Please select a URL when populated."
	$menuitem = (Get-SPOSite -IncludePersonalSite $true -Limit all -Filter "Url -like '-my.sharepoint.com/personal/'").url
	$url = Write-Menu -Title 'Sharepoint URL to Select' -Entries $menuitem
	Set-SPOSite -Identity $url -ConditionalAccessPolicy AllowLimitedAccess
	Write-Host "Conditional Access Policy has been created for : $url"
}
else {
	Write-Host "Error: Please specify either -Multimode or -Singlemode when executing script."
	Write-Host "ex. Sharepoint_Policy_Creator.ps1 -MultiMode"
	Exit
}
#Get-SPOSite -IncludePersonalSite $true -Limit all -Filter "Url -like '-my.sharepoint.com/personal/'" | Out-GridView | Set-SPOSite -ConditionalAccessPolicy AllowLimitedAccess
